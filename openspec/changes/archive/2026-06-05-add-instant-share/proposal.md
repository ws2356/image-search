...
- When the user taps a discovered PC, the Share Extension performs the full trust + transfer flow natively as an HTTP client to the PC-hosted service: calls `/trust/handshake` (includes bootstrap data), receives encrypted PIN from `/trust/apply`, sends user confirmation via `/trust/confirm`, and uploads payload via `/transfer/text` or `/transfer/image`. No local server, no bootstrap endpoint. No main-app navigation.
...
- Add a first-share trust establishment flow: after user selects a PC, mobile and PC perform DH exchange, receiver shows a PIN popup, sender shows same PIN for user confirmation. (X509 certificate exchange for long-term trust and HTTPS transport deferred — current implementation uses plain HTTP with session-key encryption for trust envelopes only.)
- HTTPS transport with self-signed certs after first trust establishment, using exchanged certificates during TLS negotiation — **deferred**.
- Signed PC mDNS TXT record and public-key pinning for future sharing — **deferred**.
...
- Keep the v1 desktop transfer direction explicit: PC hosts all trust and transfer HTTP endpoints on port 9527, and iOS acts as the HTTP client. (Initial design had iOS hosting — inverted to PC-hosted in `pc-hosted-trust-and-upload` change.)
...
- `instant-share-secure-discovery-trust`: Discover candidate PCs over mDNS (Bonjour), establish trust with PIN-verified DH. X509 certificate exchange / signed-advertisement / cert-pinned HTTPS for future sends deferred.
...
- Affected systems:
  - iOS companion app: Share Extension entrypoint, payload extraction, production device selector card (no AuBackup handoff — extension handles full flow natively)
...
- Potential dependency/API impacts:
  ...
  - Bootstrap data merged into `/trust/handshake` body; no dedicated bootstrap endpoint
