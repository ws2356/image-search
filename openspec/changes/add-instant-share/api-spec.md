...
- Session bootstrap rule:
  - The Share Extension presents the BLE-discovered device list and opens AuBackup when the user selects a PC.
  - Session is created in AuBackup after it resumes selected device and payload context from the Share Extension handoff.
  - AuBackup writes session bootstrap data to BLE `ConnectionConfig` characteristic.
