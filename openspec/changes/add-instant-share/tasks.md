...
- [ ] 1.5 Implement session bootstrap via BLE `ConnectionConfig` write (session id + mobile port + mobile ip list) and remove `/sessions` endpoint dependency.
...
- [ ] 3.5 Implement finalized desktop receive UX behavior based on selected variant.
...
- [ ] 5.3 Enforce single active instant-share session handling (reject/defer concurrent requests).
- [ ] 5.4 Add user-visible wait/abort controls for long transfers and surface user-aborted outcomes in desktop and iOS states.
