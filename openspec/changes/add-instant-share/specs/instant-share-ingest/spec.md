...
The system SHALL accept Share Extension payloads of type text, image, and video, and normalize them into a common instant-share transfer envelope containing content type, payload metadata, and source app context.
...
The system SHALL reject payload types outside text/image/video with a user-visible unsupported-type error and SHALL NOT start transfer negotiation.

#### Scenario: Reject unsupported file type
- **WHEN** the user attempts to share a non-supported attachment type (for example PDF) to the Share Extension
