...
class QRTriggerHandler:
    ...
    def _process_trigger(self, body: dict[str, object]) -> dict[str, object]:
        ...
        if payload_type == "text":
            ...
            content = body.get("content")
            if not content or not isinstance(content, str):
                return {"_status": 400, "status": "error", "error": "Missing or invalid 'content' for link type"}
            stash = self._create_text_stash(content_type="text/uri-list", content=content)
        ...
        if self._trust_session_registry is not None:
            from instant_sharing.trust_server import TrustFlowType
    ...
    def _create_file_stash(self, files_raw: list) -> StashEntry:
        ...
        from instant_sharing.contracts import FileEntry
