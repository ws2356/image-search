...
|-----------|----------|------|
| BLE Daemon | `instant_sharing/ble.py` | Always-on GATT service broadcasting PC identity via `bless` |
| Orchestrator | `instant_sharing/orchestrator.py` | Session lifecycle, trust, payload receive |
| HTTP Client | `instant_sharing/http_client.py` | PC-side HTTPS caller with TLS pinning |
| Delivery Service | `instant_sharing/delivery.py` | Routes payloads to clipboard or files |
| Sender Identity | `instant_sharing/sender_validation.py` | Ed25519 signing identity management |
| Trust Crypto | `instant_sharing/trust_crypto.py` | AES-GCM trust session envelope |
| Security | `instant_sharing/security.py` | X25519 DH + Ed25519 session signing |
| Runtime | `instant_sharing/runtime.py` | Composition root wiring all components |
