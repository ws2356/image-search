...
- **iOS package tests**: Run `cd mobile/ios && swift test --quiet`. The USB functional challenge test expects a Python environment where `python3.10` can import `websockets.sync.client`; you can use the same venv used by the PC side.
