...
- **iOS package tests**: Run `cd mobile/ios && xcodebuild test -project AlbumTransporterApp.xcodeproj -scheme AlbumTransporterApp -destination "platform=iOS Simulator,name=iPhone 17 Pro Max" -skip-testing:AlbumTransporterAppSnapshotTests/AlbumTransporterAppSnapshotTests`. The USB functional challenge test expects a Python environment where `python3.10` can import `websockets.sync.client`; you can use the same venv used by the PC side.
