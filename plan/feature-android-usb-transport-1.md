...
| TASK-040 | Add one-command pipeline scripts `dt_image_search/scripts/poc_aoa_pipeline_macos.sh` and `dt_image_search/scripts/poc_aoa_pipeline_windows.ps1` to run host POC, summary, and gate sequentially. |  |  |
...
- **TEST-009**: Run pipeline scripts in simulate mode to verify end-to-end command wiring and required-host gate behavior.
